...
export default function EditarGestantePage() {
  ...
  const fetchGestante = async () => {
    try {
      ...
      if (response.ok) {
        const gestante: GestanteDetail = await response.json()
        setFormData({
          nome: gestante.nome,
          dataNascimento: gestante.dataNascimento ? format(new Date(gestante.dataNascimento), "yyyy-MM-dd") : "",
          cartaoSus: formatCartaoSus(gestante.cartaoSus || ""),
          telefone: formatTelefone(gestante.telefone || ""),
          ubsId: gestante.ubsId || "",
          dum: format(new Date(gestante.dum), "yyyy-MM-dd"),
          dpp: format(new Date(gestante.dpp), "yyyy-MM-dd"),
          semanasInicioPreNatal: gestante.semanasInicioPreNatal.toString(),
          etnia: gestante.etnia,
          gestacoesAnteriores: gestante.gestacoesAnteriores.toString(),
          risco: gestante.risco,
          descricaoRisco: gestante.descricaoRisco || "",
          motivoAltoRisco: gestante.motivoAltoRisco || "",
          exames: {
            rotina1: gestante.exameRotina1Trimestre,
            rotina2: gestante.exameRotina2Trimestre,
            rotina3: gestante.exameRotina3Trimestre,
            ultrassom1: gestante.ultrassom1Trimestre,
            ultrassom2: gestante.ultrassom2Trimestre,
            ultrassom3: gestante.ultrassom3Trimestre,
          },
          vacinas: {
            hb: gestante.vacinaHB,
            dt: gestante.vacinaDT,
            influenza: gestante.vacinaInfluenza,
            covid19: gestante.vacinaCovid19,
            dtpa20Semana: gestante.vacinaDTPA20Semana,
            vsr28Semana: gestante.vacinaVSR28Semana,
          },
          testesRapidos1Trimestre: gestante.testesRapidos1Trimestre,
          testesRapidos3Trimestre: gestante.testesRapidos3Trimestre,
          numeroConsultas: String(gestante.numeroConsultas ?? 0),
          ...
          statusAAE: gestante.statusAAE || "nao_encaminhada",
          dataEncaminhamentoAAE: gestante.dataEncaminhamentoAAE
            ? format(new Date(gestante.dataEncaminhamentoAAE), "yyyy-MM-dd")
            : "",
          dataAcessoAAE: gestante.dataAcessoAAE
            ? format(new Date(gestante.dataAcessoAAE), "yyyy-MM-dd")
            : "",
          desfecho: gestante.desfecho || "em_andamento",
          dataDesfecho: gestante.dataDesfecho
            ? format(new Date(gestante.dataDesfecho), "yyyy-MM-dd")
            : "",
        })
      } else {
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      alert("Erro ao excluir gestante. Tente novamente.")
    }
  }
    ...
    <div className="max-w-4xl mx-auto space-y-6">
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-4">
          ...
          <div>
            <h1 className="text-3xl font-bold text-gray-900">Editar Gestante</h1>
            <p className="text-gray-600 mt-1">Atualize os dados da gestante</p>
          </div>
        </div>
          ...
          <CardContent className="space-y-4">
            ...
            <div className="space-y-2">
              ...
              {isAdmin ? (
                ...
                <Input
                  id="ubsId"
                  value={ubsList.find((u) => u.id === formData.ubsId)?.nome ?? "—"}
                  disabled
